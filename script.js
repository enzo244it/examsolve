...
// Update UI based on authentication status (similar to your example)
function updateAuthUI(user) {
    console.log('Updating auth UI:', user ? 'User logged in' : 'User logged out');
    ...
    if (user) {
        ...
        const authModalBtn = document.getElementById('authModalBtn');
        if (authModalBtn) {
            authModalBtn.textContent = 'Dashboard';
        }
        ...
        // Show all sections
        document.querySelector('.hero')?.style.setProperty('display', '');
        document.querySelector('.how-it-works')?.style.setProperty('display', '');
        document.querySelector('.credit-system')?.style.setProperty('display', '');
        document.querySelector('.pricing')?.style.setProperty('display', '');
        document.querySelector('.faq')?.style.setProperty('display', '');
        document.querySelector('.final-cta')?.style.setProperty('display', '');
        
        // Update nav button
        const authModalBtn = document.getElementById('authModalBtn');
        if (authModalBtn) {
            authModalBtn.textContent = 'Sign Up / Log In';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const checkFirebase = setInterval(() => {
        if (window.firebaseAuthFunctions) {
            ...
            // Monitor auth state changes (similar to your example)
...
function handleLoginSuccess(userData) {
    // Use updateAuthUI for consistency
...
async function handleLogout() {
    ...
    // Reset credits
    userCredits = CONFIG.DAILY_FREE_CREDITS;
    creditsUsedToday = 0;
    
    // Use updateAuthUI to handle UI updates
    updateAuthUI(null);
